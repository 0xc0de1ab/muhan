...
*/
 ...
 INVINCIBLE) && (F_ISSET(ply_ptr, PHIDDN))) {
    ...
    if(rom_ptr->lolevel > ply_ptr->level && ply_ptr->class <INVINCIBLE/*CARETAKER*/) {
        ...
        print(fd, "±×°÷À¸·Î °¥·Á¸é");
        ANSI(fd, YELLOW);
        print(fd," ·¹º§ %d", rom_ptr->hilevel);
        ANSI(fd, NORMAL);
        print(fd, "ÀÌÇÏ¿©¾ß¸¸ ÇÕ´Ï´Ù.");
        return(0);
    }
...
cmd             *cmnd;
{
 ...
 /* ÆíÁö ¼Ò°¢  */
    ...
    if(mrand(1,3000)==1) {
	  ...
	  print(fd,"\n½ÅÀÌ ´ç½ÅÀÇ Á¤¼ºÀÌ °¼¸¤ÇØ¼­ ¾öÃ»³­ °æÇèÄ¡¿Í µ·º­¶ôÀ» ³»¸³´Ï´Ù.");
	  broadcast_all("\n### ½ÅÀÌ %M¿¡°Ô ¾öÃ»³­ °æÇèÄ¡¿Í µ·º­¶ôÀ» ³»¸³´Ï´Ù.\n",ply_ptr);
