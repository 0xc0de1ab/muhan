...
cmd             *cmnd;
{
		...
		print(fd, "자객");
		ANSI(fd, NORMAL);
	    	print(fd, "만 사용할 수 있는 기술입니다.\n");
                return(0);
        }
        if(ply_ptr->class >= INVINCIBLE && !S_ISSET(ply_ptr, STHIEF) && !S_ISSET(ply_ptr, SASSASSIN)) {
                ANSI(fd, CYAN);
		print(fd, "\n자객");
		ANSI(fd, WHITE);
		ANSI(fd, NORMAL);
	...
	if(ply_ptr->class >= INVINCIBLE && ply_ptr->ready[WIELD-1]->shotscur > 0)
...
cmd             *cmnd;
{
  ...
  int     fd, i, j, k=0, fail = 0, bit[4];
  char *iname;
  int up_num=0;
  fd = ply_ptr->fd;
  if( ply_ptr->class >= INVINCIBLE && cmnd->num > 1) {
    print(fd, "무적수련을 하시려면 \"무적수련\"을 치세요.\n");
    return(0);
  }
  ...
  if(ply_ptr->class == CARETAKER) {
	if(!S_ISSET(ply_ptr, TRAINBUL)) {
		print(fd, "아직 당신은 모든능력치를 향상하지 않았습니다.\n");
		return(0);
	}
	}
  ...
  if((ply_ptr->level==100) && (ply_ptr->class < INVINCIBLE)) {
    S_SET(ply_ptr,ply_ptr->class + 99);
    ply_ptr->class = INVINCIBLE;
    if(F_ISSET(ply_ptr, PFAMIL)) {
      edit_member(ply_ptr->name, ply_ptr->class, ply_ptr->daily[DL_EXPND].max, 3);
    }
    ply_ptr->level = 1;
    ply_ptr->experience = 0;

    print(fd, "\n당신은 무적 %s 기술을 사용할 수 있게 되었습니다.\n",
	  class_str[ply_ptr->class + SASSASSIN +1]);
    broadcast_all("\n### %s님께서 무적이 되셨습니다!!",ply_ptr->name);
  }
  
  /* 초인전환 */
  if((ply_ptr->level>=127) && (ply_ptr->class == INVINCIBLE)) {
    for(j=0 ; j<8 ; j++) {
      if(!S_ISSET(ply_ptr, j+SASSASSIN)) {
	strcat(iname, class_str[j+1]);
	strcat(iname, ", ");
	k++;
      }
    }
    ...
    if((ply_ptr->level>=127) && (ply_ptr->class == CARETAKER)) {
    for(j=0 ; j<8 ; j++) {
      if(!S_ISSET(ply_ptr, j+SASSASSIN)) {
	strcat(iname, class_str[j+1]);
	strcat(iname, ", ");
...
ply_ptr->daily[DL_EXPND].max, 3);
    ...
    } while(expneeded <= ply_ptr->experience && goldneeded <= ply_ptr->gold && ply_ptr->level < MAXALVL-1 );
...
/* 빙의 걸린상태    F_SET(obj_ptr,OENCHA);    */
    ...
    case 4:
        ...
        switch(low(str[0])) {
              ...
              case '4':
                   obj_ptr->shotsmax = 700;
                   obj_ptr->shotscur = 700;
                   sum += 4000000;
                   break;

              case '5':
                   obj_ptr->shotsmax = 900;
                   obj_ptr->shotscur = 900;
                   sum += 5000000;
                   break;
...
/* 이기능은 적의 도력을 막는 루틴이다.                                */
/*                                                                    */
long magic_stop_time[PMAX]; /* 혈도 봉쇄 딜레이 타임 */
...
cmd            *cmnd;
{
	...
	/* 대응할 몹 찾기 */
        ...
        /* 혈도 딜레이 타임 설정 */
	t = time(0);
        if(magic_stop_time[fd] > t) {
                please_wait(fd, magic_stop_time[fd]-t);
                return(0);
        }
	...
	if (ply_ptr->class >= BULSA) 
	    	magic_stop_time[fd] = t + 16L;
  	else if (ply_ptr->class == CARETAKER)
	    	magic_stop_time[fd] = t + 18L;
  	else
	    	magic_stop_time[fd] = t + 20L;
	
        chance = (((ply_ptr->level+3)/4) - ((crt_ptr->level+3)/4))*20 + bonus[ply_ptr->dexterity]*6;
        chance = MIN(chance, 80);
	/* if(ply_ptr->class == RANGER) chance += 10; <- Garbagge Code */
	...
	if (mrand(20,100) <= chance) {
	    	if (ply_ptr->class >= BULSA)
		    	dmg = mrand(1, ply_ptr->class) * (crt_ptr->hpcur / 20);
	    	else if (ply_ptr->class == CARETAKER)
		    	dmg = mrand(3, ply_ptr->class) * (crt_ptr->hpcur / 20);
		else
			dmg = mrand(5, ply_ptr->class) * (crt_ptr->hpcur / 20);
		
                print(fd,"%M의 급소를 짚어서 %d의 피해를 입혔습니다.\n",  crt_ptr, dmg );
                broadcast_rom(fd, ply_ptr->rom_num, "%M이 %m의 급소를 짚어서 %d의 피해를 입혔습니다.\n", ply_ptr, crt_ptr, dmg);
                crt_ptr->hpcur -= dmg;
...
/*                       중독                                         */
/**********************************************************************/
long poison_mon_time[PMAX]; /* 독살포 딜레이 타임 */
...
cmd            *cmnd;
{
	...
	if(F_ISSET(ply_ptr, PINVIS)) {
        ...
        t = time(0);
        if(poison_mon_time[fd] > t) {
                please_wait(fd, poison_mon_time[fd]-t);
                return(0);
        }
	...
	if (ply_ptr->class >= BULSA) 
	    	poison_mon_time[fd] = t + 16L;
	else if (ply_ptr->class == CARETAKER)
	    	poison_mon_time[fd] = t + 18L;
	else
	    	poison_mon_time[fd] = t + 20L;
	...
	print(fd, "\n당신은 적에게 독을 뿌렸습니다.\n%M의 몸이 중독되었습니다.\n ", crt_ptr);
        ...
        if (mrand(20,100) <= chance) {
        	if (ply_ptr->class >= BULSA)
		    	dmg = mrand(1, ply_ptr->class) * (crt_ptr->hpcur / 40);
	    	if (ply_ptr->class == CARETAKER)
		    	dmg = mrand(3, ply_ptr->class) * (crt_ptr->hpcur / 40);
		else
			dmg = mrand(5, ply_ptr->class) * (crt_ptr->hpcur / 40);
                print(fd,"%M을 중독을 시켜서 %d의 피해를 입혔습니다.\n", crt_ptr, dmg );
                broadcast_rom(fd, ply_ptr->rom_num, "%M이 %m을 중독을 시켜서 %d의  피해를 입혔습니다.\n", ply_ptr, crt_ptr, dmg);
                crt_ptr->hpcur -= dmg;
                m = MIN(crt_ptr->hpcur, dmg);
                add_enm_dmg(ply_ptr->name, crt_ptr, m);
                if(crt_ptr->hpcur < 1) {
        ...
        if((crt_ptr->type == PLAYER && F_ISSET(crt_ptr, PRMAGI)) || (crt_ptr->type != PLAYER && (F_ISSET(crt_ptr, MRMAGI) || F_ISSET(crt_ptr, MRBEFD))))
        	dur = 3;
        else
                dur = MAX(5, dur);
