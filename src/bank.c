...
int load_family_bank(str, obj_ptr)
char	*str;
object 	**obj_ptr;
{
  int fd;
  ...
  sprintf(file, "%s/family/bank/%s", PLAYERPATH, str);
  fd = open(file, O_RDONLY | O_BINARY, 0);
  if(fd < 0) {
    return(-1);
  }
...
int save_family_bank(str, obj_ptr)
char	*str;
object  *obj_ptr;
{
  int fd;
...
cmd		*cmnd;
{
	...
	n = load_bank(cmnd->str[1], &bnk_ptr);
	if(n < 0) {

*/		
int family_bank_inv(ply_ptr, cmnd)
creature	*ply_ptr;
cmd      	*cmnd;
{
  object	*bnk_ptr;
  ...
  if(!(F_ISSET(ply_ptr->parent_rom, RDEPOT) && F_ISSET(ply_ptr->parent_rom, RFAMIL))) {
    print(fd, "패거리 창고에서만 가능합니다.");
    return(0);
  }
  
  if(cmnd->num < 2) {
    sprintf(bank, "%s_%d",family_str[ply_ptr->daily[DL_EXPND].max], rom_ptr->special); 
    n = load_family_bank(bank, &bnk_ptr);
    if(n < 0) {
      print(fd, "기증된 물건이 없습니다.");
      bnk_ptr = (object *)malloc(sizeof(object));
      zero(bnk_ptr, sizeof(object));
      bnk_ptr->shotsmax = 200;
      F_ISSET(bnk_ptr, OCONTN);
      ...
      print(fd, "패거리 창고의 기증품입니다.\n");
      strcpy(str, "기증품의 목록 : ");
      n = list_obj(&str[16], ply_ptr, bnk_ptr->first_obj);
      if(n)	print(fd, "%s.\n", str);
      else  	print(fd, "패거리에 기증된 물건이 없습니다.");
      free_obj(bnk_ptr);
    }
    return(0);
  }
  else {
    input_family_bank(ply_ptr, cmnd);
  }
  return(0);
}
...
cmd *cmnd;
{
  ...
  fd = ply_ptr->fd;
  rom_ptr = ply_ptr->parent_rom;
  if(!(F_ISSET(ply_ptr->parent_rom, RBANK) && F_ISSET(ply_ptr->parent_rom, RFAMIL) && rom_ptr->special != 0)) {
    print(fd, "패거리 금고에서만 가능합니다.");
    return(0);
  }
  ...
  print(fd, "패거리 금고의 잔고는 %d만냥입니다.", bnk_ptr->value);
  free_obj(bnk_ptr);
  return(0);
}
...
int input_family_bank(ply_ptr, cmnd)
creature	*ply_ptr;
cmd	*cmnd;
{
  int	fd, n;
  ...
  if(!F_ISSET(ply_ptr, PFMBOS)) {
    ...
    if((obj_ptr->type >= SHARP && obj_ptr->type <=MISSILE) && rom_ptr->special != 1) {
      print(fd, "\n무기류 창고에서만 가능합니다.");
      return(0);
    }
    ...
    if((obj_ptr->type >= POTION &&obj_ptr->type <= SCROLL) && rom_ptr->special != 3) {
      print(fd, "\n주술구류 창고에서만 가능합니다.");
      return(0);
    }
    if(obj_ptr->type == WAND && rom_ptr->special != 4) {
      print(fd, "\n성구류 창고에서만 가능합니다.");
      return(0);
    }
    
    if(obj_ptr->type == KEY && rom_ptr->special != 5) {
      print(fd, "\n열쇠류 창고에서만 가능합니다.");
      return(0);
    }
    
    if((obj_ptr->type == LIGHTSOURCE || obj_ptr->type == MISC || obj_ptr->type ==LOTTERY) \
       && rom_ptr->special != 6) {
      print(fd, "\n기타류 창고에서만 가능합니다.");
      return(0);
    }
  ...
  n = load_family_bank(bank, &cnt_ptr);
  if(n < 0) {
    cnt_ptr = (object *)malloc(sizeof(object));
    zero(cnt_ptr, sizeof(object));
    cnt_ptr->shotsmax = 200;
  ...
  if(cnt_ptr->shotscur >= cnt_ptr->shotsmax) {
    print(fd, "패거리 창고 안에 더이상 넣을 수 없습니다.\n");
    return(0);
  }
  
  if(F_ISSET(obj_ptr, OCONTN)) {
    print(fd, "보따리 종류는 기증할 수 없습니다.\n");
    return(0);
  }
  ...
  add_obj_obj(obj_ptr, cnt_ptr);
  cnt_ptr->shotscur++;
  print(fd, "당신은 %1i%j 기증했습니다.\n", obj_ptr,"3");
  broadcast_rom(fd, rom_ptr->rom_num, "\n%M이 %1i%j 기증했습니다.",
		ply_ptr, obj_ptr,"3");
  savegame_nomsg(ply_ptr);
  save_family_bank(bank, cnt_ptr);
  free_obj(cnt_ptr);
  return(0);
}
...
int output_family_bank(ply_ptr, cmnd)
creature	*ply_ptr;
cmd	*cmnd;
{
  int	fd, n;
  ...
  if(rom_ptr->special == 0 && !F_ISSET(ply_ptr, PFMBOS)) {
  ...
  cnt_ptr->shotscur--;
  del_obj_obj(obj_ptr, cnt_ptr);
  print(fd, "당신은 %1i%j 반출했습니다.", obj_ptr,"3");
...
int family_deposit(ply_ptr, cmnd)
creature *ply_ptr;
cmd	*cmnd;
{
  int fd, n;
  ...
  if(!(F_ISSET(ply_ptr->parent_rom, RBANK) && F_ISSET(ply_ptr->parent_rom, RFAMIL))) {
    print(fd, "패거리 금고에서만 가능합니다.");
    return(0);
  }
  
  if(cmnd->num != 2) {
    print(fd, "얼마를 기부하시려고요?");
    return(0);
  }
  
  len = strlen(cmnd->str[1]);
  if(len>4 && !strcmp(&cmnd->str[1][len-4],"만냥")) {
    amt = atol(cmnd->str[1]);
    if(amt < 1) {
    ...
    if(amt > 1000) {
      print(fd, "기부는 1000만냥 이하만 가능합니다.\n");
      return(0);
    }

    if( amt > ply_ptr->gold/10000) {
      print(fd, "당신은 그만큼의 돈을 가지고 있지 않습니다.\n");
      return(0);
    }
    /*	input_bank_all : */
    sprintf(bank, "%s_0",family_str[ply_ptr->daily[DL_EXPND].max]); 
    n = load_family_bank(bank, &bnk_ptr);
    if(n < 0) {
      bnk_ptr = (object *)malloc(sizeof(object));
      zero(bnk_ptr, sizeof(object));
    ...
    /* By Latok */
    if( bnk_ptr->value + amt * 10000 > 1000000000){
      print(fd,"\n패거리 금고의 총액은 10억 이상 될수 없습니다. \n");
      return(0);
    }
    
    bnk_ptr->value += amt;
    ply_ptr->gold -= amt * 10000;
    print(fd, "당신은 %ld만냥을 기부했습니다.\n", amt);
    print(fd, "패거리 금고의 총액이 %ld만냥이 되었습니다.", bnk_ptr->value);
    if ( amt >= 100)
      broadcast_all("\n### %s님이 패거리에 %d만냥을 기부하였습니다.",ply_ptr->name, amt);
...
int family_withdraw(ply_ptr, cmnd)
creature *ply_ptr;
cmd	*cmnd;
{
  int fd, n;
  ...
  if(len>4 && !strcmp(&cmnd->str[1][len-4],"만냥")) {
    ...
    if(amt > bnk_ptr->value) {
      print(fd, "패거리 금고에 그만큼의 돈이 없습니다.");
      return(0);
    }
    
    bnk_ptr->value -= amt;
    ply_ptr->gold += amt * 10000;
    print(fd, "당신은 %ld만냥을 인출했습니다.\n", amt);
    print(fd, "패거리 금고가 %ld만냥이 되었습니다.", bnk_ptr->value);
    save_family_bank(bank, bnk_ptr);
    free_obj(bnk_ptr);
    savegame_nomsg(ply_ptr);
  }
