  ...
  if(ply_ptr->class >= INVINCIBLE && !S_ISSET(ply_ptr, SCLERIC) && !S_ISSET(ply_ptr, SPALADIN)) {
    ...
    return(0);
        ...
        if(mrand(1,100) <= chance) {
                ...
                ply_ptr->lasttime[LT_PRAYD].interval = 500L;
        }
        else {
...
cmd             *cmnd;
{
  ...
  if(ply_ptr->class < INVINCIBLE && !(ply_ptr->class == BARBARIAN && ply_ptr->level >= 50)) {
    print(fd, "권법가 레벨 50이상만 쓸수 있는 기술입니다.\n");
    return(0);
  }
  if(ply_ptr->class >= INVINCIBLE && !S_ISSET(ply_ptr, SBARBARIAN)) {
    print(fd, "아직 권법가를 무적수련하지 않았습니다.\n");
    return(0);
  }
  ...
  chance = MIN(85, ((ply_ptr->level+3)/4)*5 + bonus[ply_ptr->dexterity]);
...
cmd             *cmnd;
{
  ...
  if(ply_ptr->class >= INVINCIBLE && !S_ISSET(ply_ptr, SPALADIN) && !S_ISSET(ply_ptr, SCLERIC)) {
