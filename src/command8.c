...
cmd             *cmnd;
{
  ...
  if(ply_ptr->class >= INVINCIBLE && !S_ISSET(ply_ptr, SFIGHTER) && !S_ISSET(ply_ptr, SBARBARIAN)) {
...
cmd             *cmnd;
{
  ...
  if(!ply_ptr->ready[WIELD-1] || (ply_ptr->ready[WIELD-1]->type
    ...
    print(fd, "맹공을 하시려면 도나 검종류의 무기가 필요합니다.");
    return(0);
  }
  
  t = time(0);
  
  if(ply_bash_time[fd] > t) {
    please_wait(fd, ply_bash_time[fd] -t);
    return(0);
  }
  ply_ptr->lasttime[LT_ATTCK].ltime = t;

  if (ply_ptr->class >= BULSA) 
     	ply_bash_time[fd] = t + 3;
  else if (ply_ptr->class == CARETAKER)
     	ply_bash_time[fd] = t + 4;
  else if (ply_ptr->class == FIGHTER)
        ply_bash_time[fd] = t + 3;
  else 
     	ply_bash_time[fd] = t + 5; 
  ...
  if(mrand(1,100) <= chance) {
    ...
    if(mrand(1,20) >= n) {
      ...
      crt_ptr->lasttime[LT_BEFUD].ltime = t;
      crt_ptr->lasttime[LT_BEFUD].interval = mrand(5,10);
      if (crt_ptr->type == MONSTER)
	F_SET(crt_ptr, MBEFUD);
      
      n = mdice(ply_ptr->ready[WIELD-1]) * 3 + mdice(ply_ptr) * 2;
	...
	if(ply_ptr->ready[WIELD-1]) {
      ...
      n = MIN(crt_ptr->hpcur /3, n);
      if(crt_ptr->class > CARETAKER) n =1;
      
      crt_ptr->hpcur -= n;
      
      print(fd, "당신의 칼을 휘둘러 %d점의 맹공을 가했습니다.\n", n);
      broadcast_rom2(fd, crt_ptr->fd, crt_ptr->rom_num,
		     "\n%M이 칼을 휘둘러 %m에게 맹공을 가합니다.", ply_ptr, crt_ptr);
      print(crt_ptr->fd, "%M이 칼을 휘둘러 %d점의 맹공을 가했습니다.\n",
	    ply_ptr, n);
      
      ply_bash_time[fd] = t+ (8 - MIN(5,ply_ptr->dexterity/5));
...
cmd             *cmnd;
{
	...
	if (ply_ptr->class > 0 && ply_ptr->level > 6) {
...
cmd         *cmnd;
{
	...
	if(F_ISSET(obj_ptr, OEVENT) && !strcmp(obj_ptr->key[2], "이벤트")) {
		strcpy(obj_ptr->key[2], ply_ptr->name);
...
cmd             *cmnd;
{
  ...
  if(ply_kick_time[fd] > t) {
    please_wait(fd, ply_kick_time[fd] -t);
    return(0);
  }
  ...
  if (ply_ptr->class >= BULSA)
  	ply_kick_time[fd] = t + 3;
  else if (ply_ptr->class == CARETAKER)
      	ply_kick_time[fd] = t + 4;
  else if (ply_ptr->class == BARBARIAN)
      	ply_kick_time[fd] = t + 3;
  else
      	ply_kick_time[fd] = t + 5;
  ...
  chance = 50 + (((ply_ptr->level+3)/4) - ((crt_ptr->level+3)/4))*5 +
    bonus[ply_ptr->strength]*3 +
    (bonus[ply_ptr->dexterity] -bonus[crt_ptr->dexterity])*2;
  chance += ply_ptr->class==BARBARIAN ? 10:0;
  ...
  if(mrand(1,100) <= chance) {
	...
	ply_kick_time[fd] = t+ (10 - MIN(7,ply_ptr->dexterity/5));
      else 
	ply_kick_time[fd] = t+ (8 - MIN(5,ply_ptr->dexterity/5));
