...
long    t;
{
    ...
    long        i,z;
    long 	atime;
    int     rtn = 0, n, fd, p=0, chance2;
    char talkbuf[32][256];
    char namebuf[1024];
    FILE *fp;
    int maxtalk=0,len, class, chance;
	...
	if(!att_ptr) {
	  ...
	  if(mrand(1,20) >= n && !p) {
		...
		print(fd, "\n%M%j 당신의 경험치 %d 를 줄었습니다.",crt_ptr,"1",n);
	      ...
	      n = mdice(crt_ptr) - ((70-att_ptr->armor)/10);
	      if(n<1) n = 1;
	    }
	    ...
	    chance2 = MIN(80, ((att_ptr->level+3)/4) + (20-att_ptr->thaco))*2;

	    if(F_ISSET(att_ptr, PREFLECT) && mrand(1,160) <= chance2) {
	      ANSI(fd,GREEN);
	      print(fd, "\n당신은 %M의 공격을 튕겨냅니다.",crt_ptr);
	      broadcast_rom(fd, att_ptr->rom_num, "\n%M%j %M의 공격을 튕겨냅니다.",att_ptr,"1",crt_ptr);
	      if(mrand(1,300) <= chance2) {
		n = MIN(crt_ptr->hpcur, n);
		print(fd, "\n%M의 공격을 튕겨내어 %d만큼의 상처를 입혔습니다.", crt_ptr, n);
		crt_ptr->hpcur -= n;
	    ...
	    case ASSASSIN:
	      chance = (((crt_ptr->level+3)/4) - ((att_ptr->level+3)/4))*5 +
		bonus[crt_ptr->dexterity]*6;
	      chance = MIN(chance, 80);
	      
	      if(mrand(1,100) <= chance && mrand(1,100) <= 15) {
		crt_poison(crt_ptr, att_ptr);
	      }
	      break;

	    case BARBARIAN:
	      chance = 50 + (((crt_ptr->level+3)/4) - ((att_ptr->level+3)/4))*5 +
                bonus[crt_ptr->strength]*3 +
		(bonus[crt_ptr->dexterity] -bonus[att_ptr->dexterity])*2;
	      
	      chance = MIN(85, chance);
	      
	      if(mrand(1,100) <= chance && mrand(1,100) <= 20) {
		crt_kick(crt_ptr, att_ptr);
	      }
	      break;
	    ...
	    case CLERIC:
	      chance = 50 + (((crt_ptr->level+3)/4) - ((att_ptr->level+3)/4))*5 +
                bonus[crt_ptr->piety]*3 + (crt_ptr->class == PALADIN ? 15:25);
	      
	      chance = MIN(80, chance);
	      
	      if(mrand(1,100) <= chance && mrand(1,100) <= 10) {
		crt_turn(crt_ptr, att_ptr);
	      }
	      break;
	      
	    case FIGHTER:
	      chance = 50 + (((crt_ptr->level+3)/4) - ((att_ptr->level+3)/4))*3 +
		bonus[crt_ptr->strength]*3 +
		(bonus[crt_ptr->dexterity] -bonus[att_ptr->dexterity])*2;
	      chance = MIN(85, chance);
	      
	      if(mrand(1,100) <= chance && mrand(1,100) <= 15) {
		crt_bash(crt_ptr, att_ptr);
	      }
	      break;
	    ...
	    case MAGE:
	      ...
	      chance = (((crt_ptr->level+3)/4) - ((att_ptr->level+3)/4))*5 +
		bonus[crt_ptr->intelligence]*5;
	      chance = MIN(chance, 80);
	      
	      if(mrand(1,100) <= chance && mrand(1,100) <= 15) {
		crt_absorb(crt_ptr, att_ptr);
	      }
	      break;
		...
		crt_magic_stop(crt_ptr, att_ptr);
	      }
	      break;
		...
		broadcast_rom(fd, att_ptr->rom_num, "\n%M의 %S%j 산산히 부서져 버렸습니다.",
			      att_ptr, att_ptr->ready[n-1]->name,"1");
		add_obj_crt(att_ptr->ready[n-1], att_ptr);
		att_ptr->ready[n-1] = 0;
		compute_ac(att_ptr);
		compute_thaco(att_ptr);
	      }
	    }
	    ...
	    if(att_ptr->hpcur < 1) {
	      ...
	      if(F_ISSET(att_ptr, PANGEL)) {
		print(fd, "\n\n애석하게도 정령이 당신을 대신해서 죽었습니다.\n");
		ANSI(fd, WHITE);
		ANSI(fd, NORMAL);
		F_CLR(att_ptr, PANGEL);
		att_ptr->hpcur = att_ptr->hpmax;
	      }

	      if(att_ptr->hpcur > 1) break;
...
creature        *att_ptr;
{
  ...
  n = mdice(crt_ptr) * 4;
  n = MIN(att_ptr->hpcur, n);
  
  if(n > 30) {
    if(att_ptr->class > CARETAKER) n = 1;
    
    att_ptr->hpcur -= n;
    
    broadcast_rom(fd, att_ptr->rom_num,
		  "\n%m%j %M에게 발차기로 %d점의 공격을 가합니다.", crt_ptr, "1", att_ptr, n);
    print(fd, "\n%m%j 발차기로 당신에게 %d점의 공격을 가했습니다.",
	  crt_ptr, "1", n);
  }
  else {
    broadcast_rom(fd, att_ptr->rom_num, "\n%m의 발차기가 실패했습니다.", crt_ptr);
    print(att_ptr->fd, "\n%M%j  당신에게 발차기로 공격하려고 합니다.", crt_ptr, "1");
  }
...
creature        *att_ptr;
{
  ...
  obj_ptr = find_obj(crt_ptr, crt_ptr->first_obj, "검", 1);
  if(!obj_ptr) find_obj(crt_ptr, crt_ptr->first_obj, "도", 1);

  if(obj_ptr)
    n = mdice(obj_ptr) * 3 + mdice(crt_ptr);
  else 
    n = mdice(crt_ptr) * 3;
  ...
  if(n > 20) {
    ...
    broadcast_rom(fd, att_ptr->rom_num,
		  "\n%m%j %M에게 칼을 휘둘러 %d점의 맹공을 가합니다.", crt_ptr, "1", att_ptr, n);
    print(fd, "\n%m%j 당신에게 %d점의 맹공을 가합니다.", crt_ptr, "1", n);
  }
  else {
    broadcast_rom(fd, att_ptr->rom_num, "\n%m의 맹공이 실패했습니다.", crt_ptr);
    print(att_ptr->fd, "\n%M%j  당신에게 맹공으로 공격하려고 합니다.", crt_ptr, "1");
  }
  crt_ptr->lasttime[LT_ATTCK].ltime = time(0);
...
int crt_absorb(crt_ptr, att_ptr)
creature        *crt_ptr;
creature        *att_ptr;
{
  int             n, fd;
  ...
  n = ((crt_ptr->level+3)/4)*mrand(2,5);
  n = MIN(att_ptr->hpcur, n);

  if(n > 20) {
    att_ptr->hpcur -= n;
    crt_ptr->hpcur += MAX(1, n);

    if(crt_ptr->hpmax < crt_ptr->hpcur)
      crt_ptr->hpcur = crt_ptr->hpmax;

    broadcast_rom(fd, att_ptr->rom_num,
		  "\n%m%j %M의 기를 %d만큼 흡수했습니다.", crt_ptr, "1", att_ptr, n);
    print(fd, "\n%m%j 당신의 기를 %d만큼 흡수했습니다.", crt_ptr, "1", n);
  }
  else {
    broadcast_rom(fd, att_ptr->rom_num, "\n%m의 흡성대법이 실패했습니다.", crt_ptr);
    print(att_ptr->fd, "\n%M%j  당신에게 흡성대법으로 공격하려고 합니다.", crt_ptr, "1");
  }
  crt_ptr->lasttime[LT_ATTCK].ltime = time(0);
  crt_ptr->lasttime[LT_ATTCK].interval = 7;
...
int crt_magic_stop(crt_ptr, att_ptr)
creature        *crt_ptr;
creature        *att_ptr;
{
  int             n, fd;
  ...
  n = att_ptr->hpcur/mrand(2,5);
  n = MIN(att_ptr->hpcur, n);

  if(n > 50) {
    att_ptr->hpcur -= n;

    att_ptr->lasttime[LT_SPELL].ltime = time(0);
    att_ptr->lasttime[LT_SPELL].interval = n/20;

    broadcast_rom(fd, att_ptr->rom_num,
		  "\n%m%j %M의 급소를 짚어서 %d점의 피해를 입혔습니다.", crt_ptr, "1", att_ptr, n);
    print(fd, "\n%m%j 당신의 급소를 짚어서 %d점의 피해를 입혔습니다.", crt_ptr, "1", n);
  }
  else {
    broadcast_rom(fd, att_ptr->rom_num, "\n%m의 혈도봉괘가 실패했습니다.", crt_ptr);
    print(att_ptr->fd, "\n%M%j  당신에게 혈도봉쇄로 공격하려고 합니다.", crt_ptr, "1");
  }
  crt_ptr->lasttime[LT_ATTCK].ltime = time(0);
  crt_ptr->lasttime[LT_ATTCK].interval = 10;
}
/**********************************************************************/
/*                             독살포                                */
/**********************************************************************/

int crt_poison(crt_ptr, att_ptr)
creature        *crt_ptr;
creature        *att_ptr;
{
  int             n, fd;
  ...
  if(n > 30) {
    ...
    att_ptr->lasttime[LT_SPELL].ltime = time(0);
    att_ptr->lasttime[LT_SPELL].interval = n/10;
    att_ptr->lasttime[LT_ATTCK].ltime = time(0);
    att_ptr->lasttime[LT_ATTCK].interval = n/20;

    broadcast_rom(fd, att_ptr->rom_num,
		  "\n%m%j %M에게 독을 뿌려서 %d의 피해를 입혔습니다.\n%M의 몸이 중독되었습니다.", \
		  crt_ptr,"1", att_ptr, n, att_ptr);
    print(fd, "\n%m%j 당신에게 독을 뿌려서 %d의 피해를 입혔습니다.\n 당신의 몸이 중독되었습니다.", crt_ptr, "1", n);
  }
  else {
    broadcast_rom(fd, att_ptr->rom_num, "\n%m의 독살포가 실패했습니다.", crt_ptr);
    print(att_ptr->fd, "\n%M%j  당신에게 독살포로 공격하려고 합니다.", crt_ptr, "1");
  }
  crt_ptr->lasttime[LT_ATTCK].ltime = time(0);
  crt_ptr->lasttime[LT_ATTCK].interval = 5;
}
...
creature        *att_ptr;
{
  ...
  if(n > 30) {
    ...
    broadcast_rom(fd, att_ptr->rom_num,
		  "\n%m%j 부적을 하늘로 날리며 혼을 소환시키는 방혼술의 주문을 외칩니다.\n부적이 %M의 몸을 공격하며%d만큼의 타격을 입혔습니다.\n", crt_ptr,"1", att_ptr, n);
    print(fd, "\n%m%j 부적을 하늘로 날리며 혼을 소환하는 방혼술의 주문을 외칩니다.\n부적이 당신을 공격하며 %d만큼의 타격을 입혔습니다..", crt_ptr, "1", n);
  }
  else {
    broadcast_rom(fd, att_ptr->rom_num, "\n%m%j 부적을 하늘로 날리며 혼을 소환시키는 방혼술의 주문을 외칩니다.\n하지만 주문이 튕겨져 나오면서 %M이 그의 주술을 견뎌냈습니다.\n",
		  crt_ptr, "1", att_ptr);
    print(att_ptr->fd, "\n당신은 %M의 방혼술을 견뎌냈습니다.", crt_ptr);
  }
  crt_ptr->lasttime[LT_ATTCK].ltime = time(0);
  crt_ptr->lasttime[LT_ATTCK].interval = 15L;
}
...
long t;
{
    ...
    if(i > t) {
        if(i-t > 60)
            ...
            broadcast_all("\n### %d초 후에 머드를 종료합니다. 모두 나가 주십시요.", i-t);
    }
    else {
        broadcast_all("\n### 머드를 종료합니다.");
        output_buf();
        logdebug("--- 머드 종료: %d --- (%.24s)\n", Port,ctime(&c));
