...
cmd         *cmnd;
{
    ...
    paste_command(cmnd->fullstr, index);
        ANSI(fd, CYAN);
        print(fd, "당신은 이제부터 %s", ansi_title(ply_ptr, ply_ptr->description));
        ANSI(fd, CYAN);
	print(fd, "있습니다.");
	return(0);
}
...
long ply_scratch_time[PMAX];

int scratch(ply_ptr, cmnd)
creature        *ply_ptr;
cmd             *cmnd;
{
    room            *rom_ptr;
    object          *obj_ptr, *cnt_ptr;
    int             fd, n, match=0, i, j, count=0;
    long t, money=100;

    fd = ply_ptr->fd;

    if(ply_ptr->level < 20 && ply_ptr->class < INVINCIBLE) {
      print(fd, "당신의 레벨로는 복권을 긁을 수 없습니다.");
      return(0);
    }
    ...
    t=time(0);
    if(ply_scratch_time[fd]>t) {
        please_wait(fd,ply_scratch_time[fd]-t);
        return 0;
    }
   ...
   if(ply_ptr->rom_num == 1001) {   
      print(fd, "광장에서는 복권을 긁을 수 없습니다.");
      return(0);
   }
   
   if(F_ISSET(rom_ptr, RSHOPP)) {
      print(fd, "%s님 밖으로 나가서 긁어 주세요.\n", ply_ptr->name);
      return(0);
   }
    ...
    if(!obj_ptr) {
        print(fd, "당신은 복권을 갖고 있지 않습니다.\n");
        return(0);
    }
    if(!(obj_ptr->type == LOTTERY)) {
		print(fd, "그것은 복권이 아닙니다.\n");
		return(0);
    }
    del_obj_crt(obj_ptr, ply_ptr);
    print(fd, "당신은 %1i%j 긁었습니다.\n", obj_ptr,"3");
    broadcast_rom(fd, rom_ptr->rom_num, "\n%M%j %1i%j 긁었습니다.\n", ply_ptr,"1", obj_ptr,"3");

    if(obj_ptr->ndice < 5) obj_ptr->ndice = 5;
    if(obj_ptr->sdice < 1) obj_ptr->sdice = 1;

    for(i=0; i < obj_ptr->ndice; i++) {
       if(mrand(1,(i + 10 + obj_ptr->pdice)) == 1) count++;
       
    }
   
   ply_ptr->hpcur -= obj_ptr->ndice;
   ply_ptr->experience -= obj_ptr->ndice;
   ...
   if(count < 1 ) {
	 ...
	 money *=10;
      ...
      money *= obj_ptr->sdice;
      if( money < obj_ptr->value ) money = obj_ptr->value;
      
      print(fd, "\n당신은 %d등에 당첨되었습니다.\n", obj_ptr->ndice - count + 1);
      print(fd, "당신은 %ld냥을 받았습니다.\n", money);
      broadcast_rom(fd, rom_ptr->rom_num, "\n%M%j %1i 당첨금 %ld냥을 받았습니다.\n", ply_ptr,"1", obj_ptr, money);
      if (money >= 1000000) 
	broadcast_all("\n### 축하합니다. %M%j %1i 당첨금 %ld냥을 받았습니다.\n", ply_ptr,"1", obj_ptr, money);
      ply_ptr->gold += money;
      if( ply_ptr->gold > 1000000000 ) {
	 print(fd, "당신의 돈이 너무 많아 10억냥만 남겨 놓고 신이 가져갑니다.\n");
	 ply_ptr->gold = 1000000000;
      }
      ply_ptr->hpcur -= ((int)pow(2,count) + obj_ptr->ndice);
      ply_ptr->experience -= ((int)pow(2,count) + obj_ptr->ndice);
      if((ply_ptr->experience < 1) || (ply_ptr->hpcur < 1)) {
	 die(ply_ptr, ply_ptr);
      }
   }
   free_obj(obj_ptr);
   ply_scratch_time[fd]= t + 5 + (int)pow(2,count);
...
cmd		*cmnd;
{
	...
	print(fd, "%s님의 패거리 가입을 취소하였습니다.\n", crt_ptr->name);
	print(crt_ptr->fd, "당신의 패거리 가입이 취소되었습니다.\n");
