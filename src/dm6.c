...
cmd             *cmnd;
{
	...
	broadcast("\n### %S%j 잿더미가 되버렸습니다! %s에게 조의를 표하십시요.\n", Ply[cfd].ply->name,"1", F_ISSET(Ply[cfd].ply, PMALES) ? "그":"그녀");
                sprintf(file, "mv -f %s/%s/%s %s/%s/%s~~",
                   ...
                   PLAYERPATH, first_han(Ply[cfd].ply->name), Ply[cfd].ply->name);
	        sprintf(file2, "mv -f %s/%s %s/%s~~", ALIASPATH, Ply[cfd].ply->name, ALIASPATH, Ply[cfd].ply->name);
		sprintf(file3, "mv -f %s/%s %s/%s~~", BANKPATH, Ply[cfd].ply->name, BANKPATH, Ply[cfd].ply->name);	
                disconnect(cfd);
