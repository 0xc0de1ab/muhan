...
int     val;
{
  ...
  while(cp) {
    ...
    if((l >= 2) && EQUAL(cp->crt, str) && (F_ISSET(ply_ptr, PDINVI) ? 1:!F_ISSET(cp->crt, MINVIS))) {
      match++;
      if(match == val) {
	found = 1;
	break;
...
creature    *att_ptr;
{
  ...
  etag        *ep;
  ctag	*cp, *prev, *cp2;
  creature    *ply_ptr, *leader;
  room        *rom_ptr;
  object      *obj_ptr;
  char        str[2048];
  long        i, t, profloss, total, xpv;
  int     n, levels = 0, expdiv, expdiv2, lessthanzero=0, group_num=0;
  
  str[0] =0;

  /* 그룹 경험치 추가 */
  if(att_ptr->following) {
    cp2 = att_ptr->following->first_fol;
    leader=att_ptr->following;
  }
  else {
    cp2 = att_ptr->first_fol;
    leader=att_ptr;
  }

  while(cp2) {
    if(!F_ISSET(cp2->crt, PDMINV)) {
      group_num++;
    }
    cp2 = cp2->next_tag;
  }
  ...
  if(crt_ptr->type == MONSTER) {
	...
	expdiv = (crt_ptr->experience * ep->damage) /
	  MAX(crt_ptr->hpmax,1);
        ...
        if (expdiv < 0){
	    expdiv += (100000 - expdiv);
	}
	
	/* 그룹 경험치 추가 */
        expdiv2 = 0;
	if(group_num > 0 && expdiv > 0 && crt_ptr->experience != expdiv) {
	  ANSI(ply_ptr->fd, CYAN)
	    print(ply_ptr->fd, "\n\n당신은 %M%j 죽여 경험치 %d점을 받았습니다.",
				   crt_ptr,"3",expdiv);
	  if(ply_ptr->name == leader->name){
	    ANSI(ply_ptr->fd,GREEN);
	    if((expdiv + (expdiv/2)*group_num) > crt_ptr->experience * 2) {
	      if(crt_ptr->experience * 2 - expdiv > 0) {
		/* print(ply_ptr->fd, "\n그룹의 대장인 당신은 그룹경험치 %d점을 추가로 받았습니다.\n",
		      (crt_ptr->experience * 2 - expdiv) / 2); */
		print(ply_ptr->fd, "\n그룹의 대장인 당신은 그룹경험치 %d점을 추가로 받았습니다.\n",
		      expdiv);
	      }
	      expdiv2 = expdiv;
	      /* expdiv2 = (crt_ptr->experience * 2 - expdiv) / 4; */
	    }
	    else {
	      print(ply_ptr->fd,
		    "\n그룹의 대장인 당신은 그룹경험치 %d점을 추가로 받았습니다.\n",
		    (expdiv/5)*(group_num+1));
	      expdiv2 = (expdiv/5)*(group_num+1);
	    }
	    ply_ptr->experience = ply_ptr->experience + expdiv + expdiv2;
	  }
	  else {
	    ANSI(ply_ptr->fd,GREEN);
	    if((expdiv + (expdiv/3)*group_num) > crt_ptr->experience * 2) {
	      if(crt_ptr->experience * 2 - expdiv > 0) {
		/* print(ply_ptr->fd, "\n당신은 그룹경험치 %d점을 추가로 받았습니다.\n", (crt_ptr->experience * 2 - expdiv) / 2); */
		print(ply_ptr->fd, "\n당신은 그룹경험치 %d점을 추가로 받았습니다.\n", (crt_ptr->experience * 2 - expdiv) / 2); 
	      }
	      expdiv2 = expdiv;
	      /* expdiv2 = (crt_ptr->experience * 2 - expdiv) / 4; */
	  ...
	  if(expdiv >= 0) {
	    ...
	    ply_ptr->experience += expdiv;
	     ...
	     print(ply_ptr->fd, "\n당신은 %M%j 죽여 경험치 %d를 받았습니다.\n",
		 crt_ptr,"3",100000-expdiv);
	     ply_ptr->experience += (100000-expdiv);
	   }
	}
	...
	/* 고레벨 경험치 깍는거 조정 */
	if(crt_ptr->experience/15 > 200000) {
	  crt_ptr->experience -= 200000;
	}
	
	else if(crt_ptr->class > INVINCIBLE ) {
	  crt_ptr->experience -= 500000;
	}
	else {
    ...
    if(!F_ISSET(rom_ptr,RSUVIV) &&
	 ...
	 !F_ISSET(crt_ptr->ready[WIELD-1], OEVENT) && !F_ISSET(crt_ptr->ready[WIELD-1], OCURSE) && !F_ISSET(crt_ptr->ready[WIELD-1], CONTAINER)) {
	...
	if(crt_ptr->ready[i] && !crt_ptr->ready[i]->questnum &&
	   !F_ISSET(crt_ptr->ready[i], OEVENT) &&
