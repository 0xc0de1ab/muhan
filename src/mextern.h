...
/* BANK.C */
extern int load_bank(), load_family_bank(), save_family_bank(), save_bank();
extern int input_bank(),input_family_bank(), output_bank(), output_family_bank();
extern int bank_inv(), family_bank_inv(), bank(), family_bank();
extern int family_deposit(), deposit(), withdraw(), family_withdraw();
...
extern void	sock_init(), sock_loop(), accept_connect(), output_buf(), 
		...
		waiting(), child_died(), reap_children(), broadcast_ansi();
...
extern int	attack(), who(), where(), search(), hide(), set(), clear(), 
		quit(), ply_suicide(), attack_crt(), whois(), effect_flag_list();
extern void	suicide();
...
extern int 	invince_train();
extern int 	invincible_kick();
extern int 	one_kill();
extern int 	eight();
extern int 	nahan();
extern int 	red_eye();
extern int      thief_stat();
extern int      poback();
extern int      angel();

/* COMMAND14.C */
extern int      guard();
extern int      lion_scream();
extern int      bnahan();
extern int      tagu();
extern int      reflect();
extern int	shadow();	/* 분신술 */

/* COMMAND15.C */
extern int	sasal();
extern int	chang();
extern int	rm_blind2();
extern int	choi();
...
extern int	resist_magic(), know_alignment(), remove_curse(), curse();
...
extern void	merror(), lowercize(), zero(), delimit(), view_file(), logdebug(),
...
extern int 	call_war(), special_obj(), special_read();
