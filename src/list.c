...
char	Progtitle[80];
char	Str[240];
int	Rlo = 0, Rhi = 10000, Type = -1, Wear = -1,
...
void list_crt()
{
	...
	for(i=Rlo; i<=Rhi; i++) {
		...
		sprintf(Str, "%3d. %-20.20s %02d %4d %02d/%02d/%02d/%02d/%02d %03d %03d %02d %05d %04d %2dd%-2d+%-2d\n\r", i, crt.name, crt.level, crt.alignment, crt.strength, crt.dexterity, crt.constitution, crt.intelligence, crt.piety, crt.hpmax, crt.armor, crt.thaco, crt.experience, crt.gold, crt.ndice, crt.sdice, crt.pdice);
		write(Fd, Str, strlen(Str));
...
void list_obj()
{
	...
	for(i=Rlo; i<=Rhi; i++) {
		...
		sprintf(Str, "%3d. %-20.20s %06d %03d %02d +%-2d %03d %2dd%-2d+%-2d %02d %02d %02d %03d\n\r", i, obj.name, obj.value, obj.weight, obj.type, obj.adjustment, obj.shotsmax, obj.ndice, obj.sdice, obj.pdice, obj.armor, obj.wearflag, obj.magicpower, obj.questnum);
		write(Fd, Str, strlen(Str));
		if(!((i-(missing+skipped)-Rlo+1)%50)) {
...
void list_rom()
{
	...
	for(i=Rlo; i<=Rhi; i++) {
		...
		rom.name[20]=0;
		sprintf(Str, "%3d. %-20.20s %03d/%03d/%03d/%03d/%03d/%03d/%03d/%03d/%03d/%03d %03d%%\n\r", i, rom.name, rom.random[0], rom.random[1], rom.random[2], rom.random[3], rom.random[4], rom.random[5], rom.random[6], rom.random[7], rom.random[8], rom.random[9], rom.traffic);
		write(Fd, Str, strlen(Str));
		if(!((i-(missing+skipped)-Rlo+1)%50)) {
			sprintf(Str, "%c\n%4s %-20s %-39s %-4s\n\r", 12, "  # ", "Name", "Random Monsters", "Traf");
			write(Fd, Str, strlen(Str));
