...
cmd		*cmnd;
{
	...
	case 'a':
		...
		if (!strcmp(cmnd->str[3], "age")) {
		    	crt_ptr->lasttime[LT_HOURS].interval = (cmnd->val[3] - 18) * 86400L;
			return(PROMPT);
		}
	...
	case 'f':
        ...
        case 'z':
                num = cmnd->val[3];
                if(num < 1 || num > 180) return(PROMPT);
                if(S_ISSET(crt_ptr, num-1)) {
                        S_CLR(crt_ptr, num-1);
                        print(fd, "%M's spell #%d off.\n", crt_ptr, num);
                }
                else {
                        S_SET(crt_ptr, num-1);
                        print(fd, "%M's spell #%d on.\n", crt_ptr, num);
                }
                return(0);               
