...
cmd            *cmnd;
{
  ...
  if(ply_ptr->class != CLERIC && ply_ptr->class != PALADIN &&
    ...
    ANSI(fd, WHITE);
    ANSI(fd, NORMAL);
    print(fd, "을 사용할 수 있습니다.\n");
    return(0);
  }
  if(ply_ptr->class >= INVINCIBLE && !S_ISSET(ply_ptr, SCLERIC) && !S_ISSET(ply_ptr, SPALADIN)) {
    ANSI(fd, CYAN);
    print(fd, "\n불제자");
    ANSI(fd, WHITE);
    ANSI(fd, NORMAL);
  ...
  chance = (((ply_ptr->level+3)/4) - ((crt_ptr->level+3)/4))*20 +
    bonus[ply_ptr->piety]*5 + (ply_ptr->class == PALADIN ? 25:15);
  ...
  if((mrand(1,100) > 90 - bonus[ply_ptr->piety]) && F_ISSET(crt_ptr, MUNDED) ) {
    print(fd,  
    ...
    if (ply_ptr->class >= BULSA)
		dmg = mrand(1, ply_ptr->class) * (crt_ptr->hpcur / 30);
    if (ply_ptr->class == CARETAKER)
		dmg = mrand(3, ply_ptr->class) * (crt_ptr->hpcur / 30);
    else if (ply_ptr->class == INVINCIBLE)
		dmg = mrand(5, ply_ptr->class) * (crt_ptr->hpcur / 30);
    else
		dmg = crt_ptr->hpcur / 3;
...
cmd    		*cmnd;
{
	...
	if (!S_ISSET(ply_ptr, SMAGE)) {
  	...
  	t = time(0);
  	if(absorb_time[fd] > t) {
    		please_wait(fd, absorb_time[fd] - t);
    		return(0);
  	}
  	...
  	absorb_time[fd] = t + 20L;
	chance = (((ply_ptr->level+3)/4) - ((crt_ptr->level+3)/4))*20 + bonus[ply_ptr->intelligence]*5;
  	chance = MIN(chance, 80);
  	if(mrand(1,100) > chance) {
  	...
  	if (ply_ptr->class >= BULSA)
	    	dmg = mrand(1, 15) * (ply_ptr->level+3)/4;
	else if (ply_ptr->class == CARETAKER)
	    	dmg = mrand(1, 10) * (ply_ptr->level+3)/4;
	else 
	    	dmg = mrand(1, 5) * (ply_ptr->level+3)/4;
  	...
  	add_enm_dmg(ply_ptr->name, crt_ptr, m);
     	...
     	print(fd, "\n당신은 %M의 기를 %d만큼 흡수했습니다.\n", crt_ptr, dmg);
    	broadcast_rom(fd, ply_ptr->rom_num, "\n%M이 %M의 기를 %d 만큼 흡수했습니다.\n\n.\n", ply_ptr, crt_ptr, dmg);
...
int              how;
{
	  ...
	  if(ply_ptr->mpcur == ply_ptr->mpmax) {    
	    print(fd, "\n당신은 도주천 주술이 필요없습니다.\n");
	    return(0);
	  }
