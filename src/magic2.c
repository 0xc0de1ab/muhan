...
int     how;
{
  ...
  if(ply_ptr->class >= INVINCIBLE && !S_ISSET(ply_ptr, SCLERIC) && !S_ISSET(ply_ptr, SPALADIN) && how == CAST) {
    ...
    /* Cast vigor on another player or monster */
	  ...
	  if(expadd > 0 && crt_ptr->type != MONSTER && crt_ptr->name != ply_ptr->name) {
	     if(mrand(1,2) == 1) {
...
/*  if(ply_ptr->class != CLERIC && ply_ptr->class != PALADIN && ply_ptr->class < INVINCIBLE && how == CAST) {
    ...
    if(cmnd->num == 2) {
        ...
        if(how == CAST){
            heal = dice(bonus[ply_ptr->intelligence] + bonus[ply_ptr->piety] + 20, 
			((ply_ptr->class >= INVINCIBLE) ? mrand(1,(ply_ptr->level+24)/25) : 0) + 
			((ply_ptr->class == CLERIC) ? ((ply_ptr->level+3)/4)*2 + 
			 mrand(1, 1+((ply_ptr->level+3)/4)/2) : 0) + 
			((ply_ptr->class == PALADIN) ? ((ply_ptr->level+3)/4) +
			 mrand(1, 1+((ply_ptr->level+3)/4)/3) : 0) + 1,
			dice(2,6,5));
	    ply_ptr->mpcur -= 10;
	    if (F_ISSET(ply_ptr->parent_rom,RPMEXT)){
	      heal += mrand(1,10)+1;
	  ...
	  heal = dice(6,mrand(1,(ply_ptr->level+24)/25),5);
	
        heal = MAX(1, heal);

	if(ply_ptr->type != PLAYER) heal /=2;
    ...
    /* Cast mend on another player or monster */
	  ...
	  if(crt_ptr->hpcur == crt_ptr->hpmax) {
	    ...
	    return(0);
            ...
            heal = dice(bonus[ply_ptr->intelligence] + bonus[ply_ptr->piety] + 30, 
			((ply_ptr->class >= INVINCIBLE) ? mrand(1,(ply_ptr->level+19)/20) : 0) + 
			((ply_ptr->class == CLERIC) ? ((ply_ptr->level+3)/4)*2 + 
			 mrand(1, 1+((ply_ptr->level+3)/4)/2) : 0) + 
			((ply_ptr->class == PALADIN) ? ((ply_ptr->level+3)/4) +
			 mrand(1, 1+((ply_ptr->level+3)/4)/3) : 0) + 1,
	  ...
	  heal = dice(6,mrand(1,(ply_ptr->level+19)/20),10);
	
        heal = MAX(1, heal);
       
        crt_ptr->hpcur += MIN(crt_ptr->hpmax, heal);
        expadd = MAX(1, heal);
