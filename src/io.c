...
int	debug;
{
	...
	if (Tablesize > PMAX){
		Tablesize = PMAX;
		logdebug("Tablesize greater than PMAX\n");
	}
...
int	fd;
{
	...
	/*************************************************************/
print(fd, "
  무한대전 1.35 (C) 1996, 원작 MORDOR 2.5(Brett J. Vickers)");

print(fd, "\n\n **            *            **  ");
print(fd, "\n *   *  **  *     *  **  *   *   ");
print(fd, "\n *           *****           *      ");
print(fd, "\n *        **       **        *   그대 피 끓는 대한의 젊은이여.");
print(fd, "\n *       **          **      *   저 끝도 없는 고난의 광야를 가르는 그대여.");
print(fd, "\n *       **          **      *   거센 비바람에 맞서 갈라져 버린 얼굴.");
print(fd, "\n *        **       **        *   험한 길 걸어온 그대의 상처난 몸둥이.");
print(fd, "\n *           *****           *   지쳐 굳게 굳어 버린 그대, 무겁게 옮기던");
print(fd, "\n *****************************   발걸음 상록수 지나며 빛나는 눈동자..");
print(fd, "\n *          *** ***          *   그리고 절로 나오는 콧노래");
print(fd, "\n *        ***     ***        *      ");
print(fd, "\n *     ***           ***     *   피끓는 가슴안고 사라졌던 이땅에.. ");
print(fd, "\n *   ***               ***   *      ");
print(fd, "\n  ****                   ****    누구도 피할수 없는 고난과 응고의 세월");
print(fd, "\n   **                     **     떳떳하고 당당한 당신이 아름다워라.");
print(fd, "\n     **                 **          ");
print(fd, "\n        **           **          바로 그대 대한의 젊은이여..");
print(fd, "\n           **     **                ");
...
int	fd;
{
	...
	if(n<=0)
		...
		for(i=0; i<n; i++) {
			...
			if (buf[i] == 0)
			    buf[i] = '\n';
			else if (buf[i] == 13 && i != n - 1 && (buf[i + 1] == '\n' || buf[i + 1] == 0))
			    buf[i] = '\r';
			else if (buf[i] == 13)
			    buf[i] = '\n';
			...
			if ((unsigned char)buf[i] == 0xff)
			    continue;
			if (i > 0 && (unsigned char)buf[i - 1] == 0xff && ((unsigned char)buf[i] == 0xfb || (unsigned char)buf[i] == 0xfc || (unsigned char)buf[i] == 0xfe)) {
			    i++;
			    continue;
			}
		    } 
		    패치 끝 */
		    if(buf[i] > 31 || ((buf[i]&0x80) && (unsigned char)buf[i]!=155) || (buf[i] == '\n' && lastchar != '\r') || buf[i] == '\b' ||buf[i]==127) {
...
void handle_commands()
{
	...
	for(i=0; i<Tablesize; i++) {
        ...
        if(alias_buf_num[i]!=alias_buf_pos[i] && Ply[i].io->fn==command) {
			...
			if(Write_CMD) log_overwrite("command.log","%s\n",buf);
...
char 	*str;
{
  ...
  for(i=0; i<Tablesize; i++) {
    if(FD_ISSET(i, &Sockets) && Ply[i].ply)
      if(!F_ISSET(Ply[i].ply, PNOBRD) && Ply[i].ply->fd > -1) {
	ply_ptr = Ply[i].ply;
	print(i, "%s", ansi_print(ply_ptr, str));
      }
  }
}
