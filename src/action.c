...
#define AOUT1(a,b,m)    {print(fd, a, ansi_print(ply_ptr, (msg_ok)?msg:m)); \
                        broadcast_rom(fd, ply_ptr->rom_num, b, ply_ptr, ansi_print(ply_ptr, (msg_ok)?msg:m));}
...
cmd             *cmnd;
{
        ...
        else if(!strcmp(cmdlist[cmdno].cmdstr, "니다")) {
                         AOUT1("당신은 %s","\n%M이 %s","바보짓을 합니다.")
		  }
        return(0);
}
