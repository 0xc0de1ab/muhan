...
cmd             *cmnd;
{
        ...
        if(!strcmp(cmnd->str[1], "모두")) {
            ...
            print(fd, "%I%j 여자들만 입을 수 있습니다.",obj_ptr,"0");
                ...
                if(obj_ptr->shotsmax > 1500 || obj_ptr->shotscur > 1500) {
		...
		if(ply_ptr->class < INVINCIBLE && obj_ptr->questnum == 0 &&
			!F_ISSET(obj_ptr, OEVENT) && ply_ptr->level < check_ac) {
...
cmd             *cmnd;
{
        ...
        if(cmnd->num > 1) {
            ...
            print(fd, "당신은 그런 물건을 가지고 있지 않습니다.");
                ...
                if(obj_ptr->wearflag != WIELD || obj_ptr->shotscur < 1) {
            print(fd, "당신은 그것을 무장할 수 없습니다.");
                        return(0);
                }

	if((obj_ptr->type == SHARP || obj_ptr->type == THRUST) &&
		  	(obj_ptr->questnum == 0 ) && !F_ISSET(obj_ptr, OEVENT) && 
      ...
      if(F_ISSET(obj_ptr,OCLSEL))
        if(!F_ISSET(obj_ptr,OCLSEL + ply_ptr->class) && ( ply_ptr->class < INVINCIBLE)){
            ...
            print(fd, "%I%j 당신의 몸 크기와 맞지 않습니다.",
                ...
                if(!F_ISSET(obj_ptr, OEVENT) && (obj_ptr->shotsmax > 1500 || obj_ptr->shotscur > 1500)) {
                	del_obj_crt(obj_ptr, ply_ptr);
                	free_obj(obj_ptr);
                	print(fd,"당신이 무기를 쥐자 푸른 빛을 내며 사라집니다.");
                	return(0);
		...
		if(ply_ptr->class < INVINCIBLE && check_dmg > 15 &&
			!F_ISSET(obj_ptr, OEVENT) && ply_ptr->level < (check_dmg * 3) && obj_ptr->questnum == 0) {
			print(fd, "당신의 능력으로는 사용할 수 없는 무기입니다.");
			return(0);
		}
		if(F_ISSET(obj_ptr, OEVENT) && strcmp(obj_ptr->key[2], ply_ptr->name)) {
			print(fd, "다른 사람의 물건은 사용할 수 없습니다.\n");
                        print(fd, "본인 아이템인지 확인 하시고,");
                        ANSI(fd, CYAN);
                        print(fd, " [아이템] 명명 ");
                        ANSI(fd, NORMAL);
                        print(fd, "으로 이름을 기록 하십시요.\n"); 
			return(0);
		}
	 if(ply_ptr->ready[WIELD-1]) {
                     ...
                     /*   ply_ptr->ready[i] = 0; */
                        compute_ac(ply_ptr);
                        compute_thaco(ply_ptr);
               print(fd, "당신은 %1i%j 벗고, %1i%j 전투태세를 취합니다.",
	        ply_ptr->ready[WIELD-1],"3", obj_ptr,"4");
             broadcast_rom(fd, ply_ptr->rom_num,
                 "\n%M%j %1I%j 벗고, %1I%j 무장합니다."
                , ply_ptr, "1", ply_ptr->ready[WIELD-1],"3", obj_ptr,"4");         
