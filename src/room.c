...
room		*rom_ptr;
{
  ...
  if(!F_ISSET(ply_ptr, PNORNM)) {
    if(strlen(rom_ptr->name) >= 2) { 
      ANSI(fd, BLUE);
      print(fd, "♨♨♨ ");
      ANSI(fd, CYAN);
      print(fd, "%s", cut_space(rom_ptr->name));
      ANSI(fd, BLUE);
      print(fd, " ♨♨♨\n\n");
      }
    else
  ...
  if(!F_ISSET(ply_ptr, PDSCRP)) {
    ...
    while(cp) {
	 ...
	 cp->crt != ply_ptr) {
	ANSI(fd, CYAN);
	print(fd, "%s님이 ", cp->crt->name);
	print(fd, "%s", ansi_print(ply_ptr, cp->crt->description));
        ANSI(fd, CYAN);
	print(fd, "있습니다.\n");
	ANSI(fd, MAGENTA);
	 if(F_ISSET(cp->crt, PANGEL))
	    print(fd, "%M의 정령이 주위를 맴돕니다.\n", cp->crt);
        ANSI(fd, WHITE);
	ANSI(fd, NORMAL);
      }
      cp = cp->next_tag;
  ...
  while(cp) {
	...
	if(!strcmp(cp->next_tag->crt->name, cp->crt->name) &&
	   (F_ISSET(ply_ptr, PDINVI) ? 1:!F_ISSET(cp->next_tag->crt, MINVIS)) &&
