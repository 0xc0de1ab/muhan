...
int              how;
{
  ...
  if(ply_ptr->class >= INVINCIBLE && !S_ISSET(ply_ptr, SCLERIC) && how == CAST) {
    print(fd,"\n불제자를 무적수련하지 않았습니다..\n");
    return(0);
  }
...
int              how;
{
  ...
  if(ply_ptr->class >= INVINCIBLE && !S_ISSET(ply_ptr, SCLERIC) && \
     !S_ISSET(ply_ptr, SPALADIN) && how == CAST) {
  ...
  if(ply_ptr->class != BULSA) {    
    if(!dec_daily(&ply_ptr->daily[DL_FHEAL]) && how == CAST && ply_ptr->class < SUB_DM) {
      print(fd, "\n당신의 몸이 너무 피곤해 이 주술을 더 이상 펼칠 수 없습니다.\n");
      if(ply_ptr->class == CARETAKER) 
	print(fd,"다른 사용자에게 완치 주문을 사용하면 회복이 가능합니다.\n");
      return(0);
    }
	}
  ...
  /* Cast heal on another player or monster */
    ...
    if(ply_ptr->class == CARETAKER) ply_ptr->daily[DL_FHEAL].cur--;
   
    if(!dec_daily(&ply_ptr->daily[DL_FHEAL]) && how == CAST &&
       ply_ptr->class < CARETAKER &&  crt_ptr->type != MONSTER) {
      print(fd, "\n당신의 몸이 너무 피곤해 이 주술을 더 이상 펼칠 수 없습니다.\n");
      return(0);
    }
    ...
    if(how == CAST) { 
      ply_ptr->mpcur -= 50;
      if(ply_ptr->class >= CARETAKER) ply_ptr->mpcur -= 50;
    }
    if(how == CAST || how == SCROLL || how == WAND) {
      print(fd, "\n당신은 %M에게 완치부적을 먹이며 주문을 외웁니다.\n갑자기 그의 몸에서 심한 진동이 일어나면서 체력이 \n회복되는 것이 느껴집니다.", crt_ptr);
      print(crt_ptr->fd, "\n%M이 당신에게 완치부적을 먹이며 주문을 외웁니다.\n갑자기 당신의 몸에서 심한 진동이 일어나면서 체력이 \n회복되는 것이 느껴집니다.",
	    ply_ptr);
      broadcast_rom2(fd, crt_ptr->fd, ply_ptr->rom_num,
...
int              how;
{
  ...
  if(ply_ptr->class >= INVINCIBLE && !S_ISSET(ply_ptr, SRANGER) && how == CAST) {
    print(fd,"\n포졸을 무적수련하지 않았습니다..\n");
    return(0);
  }
        ...
        if(cmnd->num == 2) {
				...
				/* 배우자 확인 */
				if (strcmp(str, crt_ptr->name) == 0) goto next1;
			}
