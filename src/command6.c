...
cmd             *cmnd;
{
    ...
    if(F_ISSET(ext_ptr,XPGUAR)){
        ...
        if(rom_ptr->lolevel > ply_ptr->level && ply_ptr->class < INVINCIBLE/*CARETAKER*/) {
                ...
                print(fd, "그곳으로 갈려면 레벨");
                ANSI(fd, YELLOW);
                print(fd," %d", rom_ptr->hilevel+1);
                ANSI(fd, WHITE);
                ANSI(fd, NORMAL);
                print(fd, "이하여야만 합니다.");
                return(0);
...
cmd             *cmnd;
{
	...
	if(ply_ptr->class >= INVINCIBLE && !S_ISSET(ply_ptr, STHIEF)) {
        ...
        if(F_ISSET(obj_ptr, OEVENT)) chance =0;
        ...
        if(obj_ptr->first_obj) {
            for(otag_ptr=obj_ptr->first_obj; otag_ptr; otag_ptr=otag_ptr->next_tag) {
                ...
                if(F_ISSET(otag_ptr->obj, OEVENT) && ply_ptr->class < DM) chance=0;
            }
        }
		...
		print(fd, "당신은 %ld냥을 훔쳐 %ld냥을 갖고 있습니다.", obj_ptr->value, ply_ptr->gold);
		free_obj(obj_ptr);
		}
		else
		{
