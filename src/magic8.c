...
{          
  ...
  if(ply_ptr->class >= INVINCIBLE && !S_ISSET(ply_ptr, SCLERIC)) {
    print(fd,"\n불제자를 무적수련하지 않았습니다..\n");
    return(0);
  }
...
int      how;
{
    ...
    if(how == CAST) {
        dur =  100 + mrand(1,30)*5  + bonus[ply_ptr->intelligence]*20;
    }
    else if (how == SCROLL)
        dur =  50 + mrand(1,15)*5  + bonus[ply_ptr->intelligence]*20;
    else
        dur = 50 + mrand(1,15)*5;
...
int     how;
{
    ...
    if(ply_ptr->class < BULSA) {
       print(fd, "아직 당신의 능력으로는 외울수 없는 주문입니다."); 
                return(0);
        }
    ...
    if(!S_ISSET(ply_ptr, SRMGONG) && how == CAST) {
        print(fd, "당신은 아직 그런 주문을 터득하지 못했습니다.");
        return(0);
    }
    if(spell_fail(ply_ptr)) {
    ...
    if(cmnd->num == 2) {
        ...
        if(how == CAST || how == SCROLL || how == WAND)  {
            print(fd, "당신이 공포해소 주문을 외우자 주위에 있던 공포가 사라집니다.");
            broadcast_rom(fd, ply_ptr->rom_num,
                      "\n%M이 공포해소 주문을 위우자 그의 주위에 있던 공포가 사라집니다.",
                      ply_ptr);
        }
        else if(how == POTION & F_ISSET(ply_ptr, PFEARS))
            print(fd, "새하얗던 얼굴에 핏기가 돌기 시작합니다.");
        else if(how == POTION)
            print(fd, "아무 반응이 없습니다.");
        ...
        if(how == CAST || how == SCROLL || how == WAND) {
            print(fd, "당신은 %M의 회복을 기원하며 공포해소 주문을 외우자\n그의 공포가 사라짐을 느낍니다.\n", crt_ptr);
            broadcast_rom2(fd, crt_ptr->fd, ply_ptr->rom_num,
                       "\n%M이 %M의 회복을 기원하며 공포해소 주문을 외우자\n%M의 공포가 사라짐을 느낍니다.\n",
                       ply_ptr, crt_ptr, crt_ptr);
            print(crt_ptr->fd, "%M이 당신에게 공포해소 주문을 외우자 당신의 겁이 사라집니다.\n", ply_ptr);
        }
